...
def validate_list(data):
    valid_count = 0
    for _range, _pattern, _value in [k.split() for k in data]:
        print(_range, _pattern, _value)
        if validate_password2(_range, _pattern.strip(":"), _value):
            print("VALID")
            valid_count += 1
        else:
            print("INVALID")
...
def validate_password1(range, pattern, value):
    _min, _max = range.split("-")
    _count = value.count(pattern)
    print(f"Found {_count} occurences of {pattern} in {value}. Checking range {_min} - {_max}")
    return int(_min) <= _count <= int(_max)


def validate_password2(range, pattern, value):
    pos1, pos2 = range.split("-")
    pos1_valid = value[int(pos1) - 1] == pattern
    pos2_valid = value[int(pos2) - 1] == pattern
    valid = pos1_valid != pos2_valid
    print(
        f"Looking at index {pos1} and {pos2} of {value} for {pattern}. {pos1_valid} {pos2_valid} {valid}"
    )
    return valid


valid_count = validate_list(DATA)
