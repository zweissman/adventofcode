...
def run(program):
    ...
    while index not in index_list:
        ...
        operand, argument  = instruction.split()
        if operand == "acc":
            sign, value = argument[:1], argument[1:]
            if sign == '+':
                accumulator += int(value)
            elif sign == '-':
                accumulator -= int(value)
            else:
                raise Exception(f"Unknown argument for {operand}: {argument}")

            next_index = index + 1
        elif operand == "jmp":
            sign, value = argument[:1], argument[1:]
            if sign == '+':
                next_index = index + int(value)
            elif sign == '-':
                next_index = index - int(value)
            else:
                raise Exception(f"Unknown argument for {operand}: {argument}")
...
if __name__ == "__main__":
    ...
    for index in range(0, len(program)):
        ...
        if 'nop' in program[index]:
            # for line in program:
            #     print(line)
            print(f"-- Changing line {index}")
            program[index] = program[index].replace('nop', 'jmp')
            # for line in program:
            #     print(line)
        elif 'jmp' in program[index]:
            # for line in program:
            #     print(line)
            print(f"-- Changing line {index}")
