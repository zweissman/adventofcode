...
DATA_TEST2 = [
    "yr:1972 cid:100 hcl:#18171d ecl:amb hgt:170 pid:186cm iyr:2018 byr:1926",
    "",
            ...
            if key in REQUIRED:
                try:
                    if key == "byr":
                        ...
                        height = value[:-2]
                        uom = value[-2:]
                        if uom == "cm" and 150 <= int(height) <= 193:
                            required_count += 1
                        elif uom == "in" and 59 <= int(height) <= 76:
                            required_count += 1
                        else:
                            print(f"Unknown height: {value}")
                    elif key == "hcl":
                        if re.search(r"^#(?:[0-9a-fA-F]{3}){1,2}$", value):
                            required_count += 1
                        else:
                            print(f"Unknown hair color: {value}")
                    elif key == "ecl":
