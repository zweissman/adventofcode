...
def get_bag_count(color, data):
    bag_list = [
        " ".join(bag.split()[:2]) for bag in data if color in bag and not bag.startswith(color)
    ]
    count = len(bag_list)
    print(f"found {count} listings for {color}")
    if count != 0:
        for bag in bag_list:
            if not bag.startswith(color):
                words = bag.split()
                new_color = " ".join(words[:2])
                new_bag_list = get_bag_count(new_color, data)
                bag_list = list(set(bag_list + new_bag_list))

    return bag_list


if __name__ == "__main__":
    bag_list = get_bag_count("shiny gold", DATA)
    print(f"Bag count: {len(bag_list)}")
