...
def get_bag_count(color, data):
    bag_list = [bag for bag in data if color in bag and bag.startswith(color)]
    assert len(bag_list) == 1, f"Didn't find 1 matching bag description for {color}"

    bag_color, contains = bag_list[0].split(" bags contain ")
    print(f"{bag_color}--> {contains}")
    contain_bags = contains.split(", ")

    count = 1  # count yourself
    for contain_bag in contain_bags:
        contain_list = contain_bag.split(" ")
        contain_bag_quantity = contain_list[0]
        if contain_bag_quantity == "no":
            return count
        contain_bag_color = " ".join(contain_list[1:3])
        contain_bag_count = get_bag_count(contain_bag_color, data)
        print(f"{contain_bag_quantity}: {contain_bag_color} --> {contain_bag_count}")
        count += int(contain_bag_quantity) * contain_bag_count

    return count


if __name__ == "__main__":
    bag_count = get_bag_count("shiny gold", DATA) - 1
    print(f"Bag count: {bag_count}")
