...
def run(part: int, empty_row_count:int=1,test_run: bool = False, debug: bool = False):
    if test_run:
...
def part1(data: list[str], empty_row_count:int=1, debug: bool = False) -> int:
    ...
    if debug: print(f"{empty_rows=}")
    if debug: print(f"{empty_cols=}")
    ...
    if debug: print(stars)
    
    combos = list(itertools.combinations(range(len(stars)), 2))


    for star1, star2 in combos:
        distance = calc_distance(grid, stars[star1+1], stars[star2+1], empty_rows, empty_cols, empty_row_count)
        results += distance
...
def calc_distance(grid, star1, star2: tuple[int, int], empty_rows, empty_cols, empty_row_count: int) -> int:
...
if __name__ == "__main__":
    ...
    final = run(part=2, empty_row_count=999999, test_run=False, debug=False)  # 625243292686
    print("ANSWER:", final)
