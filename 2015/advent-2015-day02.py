...
def part1(data: list[str], debug: bool = False) -> int:
    ...
    for line in data:
        ...
        width, height, length = int(ws), int(hs), int(ls)
        slack = min(width * height, height * length, width * length)
        area = (2 * width * height) + (2 * height * length) + (2 * width * length) + slack
...
def part2(data: list[str], debug: bool = False) -> int:
    ...
    for line in data:
        ws, hs, ls = line.split("x")
        width, height, length = int(ws), int(hs), int(ls)
        slack = min(2 * width + 2 * height, 2 * height + 2 * length, 2 * width + 2 * length)
        area = (width * height * length) + slack
